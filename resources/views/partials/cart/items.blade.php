<div class="flex gap-2.5 lg:gap-5 py-3 lg:py-4.5 items-start border-b border-gray-100 last:border-0">
    <!-- Зображення: фіксований розмір без стискання -->
    ...
    <div class="flex flex-col grow min-w-0 self-stretch">
        ...
        <!-- Управління кількістю (притиснуто до низу) -->
    ...
    <div class="self-center pl-2">
        <x-button variant="ghost" color="light" size="sm" icon wire:click="remove({{ $item->id }})"
            wire:loading.attr="disabled" class="hover:bg-red-50 group">
            <x-lucide-trash-2 class="size-5 stroke-gray-500 group-hover:stroke-red-500 transition-colors"
                stroke-width="1.5" />
        </x-button>
    </div>
</div>
