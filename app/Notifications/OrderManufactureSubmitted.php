...
class OrderManufactureSubmitted extends Notification
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        return (new MailMessage)
            ...
            ->lineIf($manufacture->sheath_type, "• **Піхви:** {$manufacture->sheath_type}")
    ...
    public function toTelegram($notifiable): TelegramMessage
    {
        ...
        return TelegramMessage::create()
            ...
            ->lineIf($manufacture->blade_shape, "• <b>Форма клинка:</b> {$manufacture->blade_shape}")
            ->lineIf($manufacture->blade_steel, "• <b>Сталь:</b> {$manufacture->blade_steel}")
            ->lineIf($manufacture->blade_grind, "• <b>Спуски:</b> {$manufacture->blade_grind}")
            ->lineIf($manufacture->blade_finish, "• <b>Фініш:</b> {$manufacture->blade_finish}")
            ->lineIf($manufacture->blade_length, "• <b>Довжина клинка:</b> {$manufacture->blade_length} мм")
            ->lineIf($manufacture->blade_thickness, "• <b>Товщина обуха:</b> {$manufacture->blade_thickness} мм")
            ->lineIf($manufacture->handle_material, "• <b>Руків'я:</b> {$manufacture->handle_material} ({$manufacture->handle_color})")
            ->lineIf($manufacture->sheath_type, "• <b>Піхви:</b> {$manufacture->sheath_type} ({$manufacture->sheath_carry})")
            ->lineIf($manufacture->engraving_text, "• <b>Гравіювання:</b> {$manufacture->engraving_text}")
